...
import ModalManualRefundShippingFees from 'c/icx_orderActionManualRefundShippingFees';
...
export default class ICX_Order_highlight_Panel_LWC extends NavigationMixin(LightningElement) {
  @track error;
  orderDetailsImgSrc = imagesResource + '/images/orderPage/orderDetailsIcon.svg';
  ...
  @track
  optionsToDisplay;
  statusShippingFulfilled = ['fulfilled'];
  ...
  options() {
    ...
    let orderActions = [
      ...
      { label: 'Cancel Return', value: 'cancelReturn', disabled: true, isTwist: true },
      { label: 'Cancel', value: 'cancelShipping', disabled: true, isTwist: false }, //not put value as 'cancel' because it cause problem
      { label: 'Refund', value: 'refund', disabled: true, isTwist: true },
      { label: 'Manual Refund Shipping Fees', value: 'manual_refund_delivery_fees', disabled: true, isTwist: true },
      ...
      { label: 'Download Invoice', value: 'downloadInvoice', disabled: true, isTwist: false }];
    ...
    for (let i = 0; i < orderActions.length; i++) {
      ...
      if (!(orderActions[i].isTwist && !this.orderdetailsapi.isTwist)) {
        ...
        else if (this.hasOrderShippingCancelPermission && orderActions[i].value == 'cancelReturn') {
          // CANCEL RETURN
          orderActions[i].disabled = !this.orderdetailsapi.hasCancelReturn;

        }
      }

      //DOWMLOAD INVOICE FOR ALL TYPE OF ORDERS
      if (hasOrderActionDownloadInvoice && orderActions[i].value == 'downloadInvoice') {
      ...
      else if (hasOrderActionManualRefundShippingFeesPermission && orderActions[i].value == 'manual_refund_delivery_fees' ) {
        orderActions[i].disabled = !this.orderdetailsapi.isManualRefundShippingFees;
      }

    }
    // }
    console.log('First loading options: ' + JSON.stringify(orderActions));
    return orderActions;
  }
  ...
  async handleAction(event) {
    ...
    if (event.detail.value == 'exchange' && orderdetailsapi.account.DREAMID__c == null) {
      ...
      for (var i = 0; i < orderdetailsapi.order_lines.length; i++) {
        ...
        // *************** //
        // Action : MANUAL REFUND DELIVERY FEES //
        // *************** //
        // else if (event.detail.value == 'manual_refund_delivery_fees') {
        //   if (orderdetailsapi.order_lines[i].isManualRefundShippingFees) {
        //     products.push(orderdetailsapi.order_lines[i]);
        //   }
        // }
        // ***************** //
        ...
        else if (event.detail.value == 'downloadInvoice') {
          if (this.statusShippingFulfilled.includes(orderdetailsapi.order_lines[i].status.toLowerCase())) {
            products.push(orderdetailsapi.order_lines[i]);
          }
        }
        else if (event.detail.value == 'cancelReturn') {
      ...
      if (event.detail.value == 'declareFundsReception') {
        ...
        console.log('orderAction - ' + event.detail.value);
        orderActionResult = await ModalManualRefundShippingFees.open({
          size: 'small',
          content: 'Modal opened',
          orderdetailsapi: orderdetailsapi,
          // products: products,
