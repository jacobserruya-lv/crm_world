({
    invoke: function (component, event, helper) {
        ...
        var validTypeValues = ['object','record', 'app', 'url', 'tab', 'knowledge', 'namedpage', 'relatedlist','component'];
        if (validTypeValues.includes(destinationType)) {
            switch (destinationType) {
                case 'component' :
                    // Note: Lightning Component should implement lightning:isUrlAddressable
                    pageReference.type = 'standard__component';
                    pageReference.attributes.componentName = 'c__' + component.get("v.destinationName");
                    
                    var destinationNameParameters = component.get("v.destinationNameParameters");
                    console.log("destinationNameParameters", destinationNameParameters);
                    var paramTable = destinationNameParameters.split("&amp;");
                    ...
                    for (var i = 0; i < paramTable.length; i++) {
                        ...
                        console.log("keyValArray[0]", keyValArray[0]);
                        console.log("keyValArray[1]", keyValArray[1]);
                        
                        pageReference.state["c__" + keyValArray[0]] = keyValArray[1];
                    }
        ...
        //TODO close the existing tab

        if (component.get("v.openSubtab")) {
            console.log("component.get(v.openSubtab)", component.get("v.openSubtab"));
            helper.openSubtab(component, event, pageReference);
        } else {
            console.log("navService.navigate");
