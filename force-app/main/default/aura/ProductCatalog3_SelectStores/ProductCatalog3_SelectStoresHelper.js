({
    ...
    digitalStoreChange: function (cmp, helper) {
        ...
        action.setCallback(this, function (response) {
            ...
            if (state === "SUCCESS") {
                ...
                var event = $A.get('e.c:ProductCatalogGetDigitalStoreEvent');
                event.setParams({
    ...
    updateListStores: function (cmp, event, helper) {
        var selectedStores = helper.storeCodesToNames(cmp.get('v.selectedPhysicalStores').concat([cmp.get('v.selectedDigitalStore')]));
        var selectedPhysicalStores = helper.storeCodesToNames(cmp.get('v.selectedPhysicalStores'))[0];
        console.log('selectedPhysicalStores in SelectStore', selectedPhysicalStores);
        var selectedWarehouses = cmp.get('v.selectedWarehouses');
        var selectedZone = cmp.get('v.selectedZoneLevel');
        var selectedZone3 = cmp.get('v.selectedZoneLevel3');
        var myEvent = $A.get('e.c:ProductCatalogUpdateListStores');
        myEvent.setParams({ 'selectedStores': selectedStores, 'selectedWarehouses': selectedWarehouses, 'selectedZone': selectedZone, 'selectedCountries': selectedZone3, 'selectedPhysicalStores': selectedPhysicalStores });
        myEvent.fire();
    },
    ...
    setStoresByZone3: function (cmp, event, helper, selectedZoneStores) {
        ...
        var selectedCountryList = selectedZoneStores;
        console.log('Print here the countries found so far the second time', selectedCountryList);
        // var timer = setTimeout(function () {
        ...
        //console.log('selectedWarehouses', selectedWarehouses);
        var selectedPhysicalStores = helper.storeCodesToNames(cmp.get('v.selectedPhysicalStores'))[0];
        console.log('selectedPhysicalStores in SelectStore2', selectedPhysicalStores);
        var allSelectedStores = helper.storeCodesToNames(cmp.get('v.selectedPhysicalStores').concat([cmp.get('v.selectedDigitalStore')]));
        var selectedZone = cmp.get('v.selectedZoneLevel');
        myEvent.setParams({ 'selectedStores': allSelectedStores, 'selectedWarehouses': selectedWarehouses, 'selectedZone': selectedZone, 'selectedCountries': selectedZoneStores, 'selectedPhysicalStores': selectedPhysicalStores });
        myEvent.fire();
        // }, 100);
