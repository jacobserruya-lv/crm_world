...
const COLS = [
    ...
    { label: 'Sub UseCase', fieldName: 'Is_Sub_UseCase', type: 'boolean', sortable: false, cellAttributes: { class: { fieldName: 'Colorclass' } } },
    { label: 'Target Platform', fieldName: 'TargetPlatform', type: 'text', sortable: true, cellAttributes: { class: { fieldName: 'Colorclass' } } },
];
...
export default class BlFieldMappingManager extends LightningElement {
    ...
    initUseCases(result) {
        ...
        this.useCases.forEach(useCase => {
            ...
            useCase.FieldsMappings.forEach(function (fieldMapping, index) {
                ...
                fieldMapping.Sub_UseCase_DeveloperName = fieldMapping.Sub_UseCase !== undefined ? fieldMapping.Sub_UseCase.DeveloperName : '';
    ...
    loadTargetOptions(){
        this.targetPlatformOptions = [{label:'WECHAT', value: 'WECHAT'}, {label:'IOS', value: 'IOS'}]
    }
    ...
    handleTargetPlatformSelect(event) {
        console.log('OOOO222',event.target)
        this.fieldMappingRecord.TargetPlatform = event.target.value
    }
    ...
    saveFieldMappingRecord() {
        console.log('OOOOOOO',this.fieldMappingRecord.TargetPlatform)
        let valid = true
        ...
        if (this.fieldMappingRecord.Is_Sub_UseCase === undefined) {
            this.fieldMappingRecord.Is_Sub_UseCase = false;
        } else {
            this.fieldMappingRecord.Sub_UseCase = this.getUseCaseByDevName(this.fieldMappingRecord.Sub_UseCase_DeveloperName)
        }
        for (let i = 0; i < this.fieldOptions.length; i++) {
            ...
            if (!this.isMappingExist(this.fieldMappingRecord.SFFieldPath, this.fieldMappingRecord.Id)) {
                useCase.FieldsMappings.forEach(mapping => {
                    if (mapping.Id == this.fieldMappingRecord.Id) {
                        ...
                        mapping.Is_Sub_UseCase = this.fieldMappingRecord.Is_Sub_UseCase;
                        mapping.Sub_UseCase_DeveloperName = this.fieldMappingRecord.Sub_UseCase_DeveloperName;
                        mapping.Sub_UseCase = this.fieldMappingRecord.Sub_UseCase;
                        mapping.TargetPlatform = this.fieldMappingRecord.TargetPlatform;
    ...
    handleFormCheckboxChange(event) {
        ...
        if (event.target.name == 'Is_Sub_UseCase') {
            if (!event.target.checked) {
                this.fieldMappingRecord.Sub_UseCase = null
            }
        }
    }
    ...
    saveMapping() {
        ...
        if (useCase.FieldsMappings) {
            for (let j = 0; j < useCase.FieldsMappings.length; j++) {
                ...
                fieldMapping.Is_Sub_UseCase = fieldMapping.Is_Sub_UseCase === undefined ? false : fieldMapping.Is_Sub_UseCase;
